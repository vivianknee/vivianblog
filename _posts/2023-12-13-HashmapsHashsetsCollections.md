...
categories: ['Lesson']
tags: ['SQLite', 'lesson', 'Collections']
type: hacks
week: 15
author: Vivian
...
- a library system. The library has various collections such as fiction, non-fiction, science fiction, mystery, and biography. Each collection contains books related to a specific genre or topic, making it easier for people to locate books they want. 
...
public class ShoePrices {
    public static void main(String[] args) {
        ...
        Map<String, ArrayList<Double>> hashMap = new HashMap<>();

        ArrayList<Double> nikePrices = new ArrayList<>();
        nikePrices.add(41.97);
        nikePrices.add(80.97);
        hashMap.put("Nike", nikePrices);

        ArrayList<Double> adidasPrices = new ArrayList<>();
        adidasPrices.add(69.99);
        hashMap.put("Adidas", adidasPrices);

        ArrayList<Double> vansPrices = new ArrayList<>();
        vansPrices.add(55.00);
        hashMap.put("Vans", vansPrices);

        ArrayList<Double> nikePricesFromMap = hashMap.get("Nike");
        for (int i = 0; i < nikePricesFromMap.size(); i++) {
            System.out.println(nikePricesFromMap.get(i));
        }
    }
}
...
1. find by email
- finds by email
2. findAllByOrderByNameAsc
- finds name by ascending order
3. findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase
- find name or email without being case sensitive
4. findByEmailAndPassword
- find email and password
5. findByLikeTermNative
- find by native term
...
- many of one of a certain object can be assigned to one object. Roles can be assigned to multiple people. Different objects can have the same id
- @ManyToOne annotation is used to establish a many-to-one relationship between the Note entity and the Person entity. notes can only be applied to one assigned object. This column is used to store the reference to the associated Person.
...
- tutorial_id can be found on the right most column of the note database. 
- google defines the @JoinColumn annotation as something that specifies the column used for joining an entity association or element collection, so i assume that tutorial_id represents the foreign key relationship to the Person entity. Each row in the Note table will have a unique value in the "id" column, serving as a unique identifier for each note record. The "tutorial_id" column, on the other hand, establishes a foreign key relationship with the Person entity, linking each note to a specific person.
