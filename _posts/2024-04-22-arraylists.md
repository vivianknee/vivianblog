...
- array lists are mutable, arrays are not
...
- Answer the following questions:
    ...
    * Look back at *Size of the `ArrayList`*. What does the code output and why?
    - the code outputs 0 because nothing has been added to the array list so the size is 0
    ...
    * Look back at *Adding items to an `ArrayList`*. What does the code output and why? What type of function is `void`, and what will be the return value?
    - when you add to an arraylist, there is no code output since the function is void meaning nothing is returned. however, you can print the contents of the array list to see the elements added
    ...
    * Look back at Example 1. What two lines did we remove? Why?
    - removed the lines adding a double and boolean. Because the arraylist is established to only contain strings, other data types being added induce errors
...
public class HackHelper {
    ...
    static {
        ...
        arr.add(5);
        arr.add(8);
        result.append("Added Items: " + 1 + ", " + 2 + ", " + 4 + ", " + 5 + ", " + 8 + "\n");
    }


    public static void firstAndLastElement(ArrayList<Integer> array) {
        int first = array.get(0);
        int last = array.get(array.size() - 1);
        result.append("Size: " + array.size() + "\n");
        System.out.println("the first element is: " + first + " and the last element is: " + last);
    }

    public static void removeItem(ArrayList<Integer> array, int index) {
        result.append("Removed Item: " + array.get(index) + "\n");
        array.remove(index);
    }

    public static void replaceItem(ArrayList<Integer> array, int index, int newValue) {
        result.append("Replaced Item: " + array.get(index) + "\n");
        array.set(index, newValue);
    }

    public static void main(String[] args) {
        firstAndLastElement(arr);
        replaceItem(arr, 3, 25);
        removeItem(arr, 1);
        System.out.println(result);
    }
}
...
- The indicies of an Arraylist start at 0; If you try to use any value lower than 0, you will get an out of bounds error
...
- the method findMax is self explanatory, it finds the max value in the array of doubles. It does this by iterating though the array and re assigning the variable max till the end of the array is reached. 
...
- Answer the questions: 
    * Look back at the examples. What's similar? What's different?
    - the first uses an array while the second uses an arraylist
    - both use the for-each loop to iterate over the array/arraylist

    * Why do we use `ArrayList`? Why not just regular lists?
    - arraylists are dynamic so they're not fixed size like regular lists meaning we have more flexibility with them  (resizing)
...
```
    ...
    ArrayList before removal: [Red, Blue, Green]
    Removed element: Blue
    ArrayList after removal: [Red, Green]
...
public class ArrayListExample {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("ArrayList before insertion sort: " + numbers);
...
```
    ...
    ArrayList before insertion sort: [50, 30, 40, 10, 20]
    ArrayList after insertion sort: [10, 20, 30, 40, 50]
